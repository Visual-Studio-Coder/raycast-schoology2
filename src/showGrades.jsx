...
function getChartUrl(grades, categories) {
  ...
  const dataPoints = grades.map((grade, index) => {
    const gradesUpToIndex = grades.slice(0, index + 1).filter((grade) => grade.exception !== 1);
    ...
    categories.forEach((category) => {
      const categoryGrades = gradesUpToIndex.filter((grade) => grade.category_id === category.id);
